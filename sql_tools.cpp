...
void sql_tools::Add_some_card()
{
    QSqlQuery query_add = QSqlQuery(db);

    QString query_add_str = "CALL insert_data(0, 'МакФлай', 'Марти', 'Младший', '1980-11-09', 'Мужской', '');"
                            "CALL insert_data(1, 'Янченков', 'Никита', 'Сергеевич', '1998-03-11', 'Мужской', '');"
                            "CALL insert_data(2, 'Усов', 'Игорь', 'Николаевич', '1998-06-28', 'Мужской', '');"
                            "CALL insert_data(3, 'Павидлов', 'Сергей', 'Антонович', '1975-02-19', 'Мужской', '');"
                            "CALL insert_data(4, 'Турчинский', 'Глеб', 'Викторович', '1905-01-01', 'Мужской', '');";
...
void sql_tools::Count_of_elem()
{
    QSqlQuery query_add = QSqlQuery(db);

    QString query_add_str = "SELECT return_count_elem()";
    ...
    else
    {
        if(query_add.next())
        {
            QMessageBox::information(0, "Вывод функции", "Количество студентов: " + QString::number(query_add.value(0).toInt()));
        }
    }
}
